...
if hasattr(BME_CONST, '__dict__'):
    for key in BME_CONST.__dict__:
        value = BME_CONST.__dict__[key]
...
class BME680(BME680Data):
    ...
    def _get_calibration_data(self):
        ...
        calibration = self._get_regs(BME_CONST.COEFF_ADDR1, BME_CONST.COEFF_ADDR1_LEN)
        calibration += self._get_regs(BME_CONST.COEFF_ADDR2, BME_CONST.COEFF_ADDR2_LEN)

        heat_range = self._get_regs(BME_CONST.ADDR_RES_HEAT_RANGE_ADDR, 1)
        heat_value = BME_CONST.twos_comp(self._get_regs(BME_CONST.ADDR_RES_HEAT_VAL_ADDR, 1), bits=8)
        sw_error = BME_CONST.twos_comp(self._get_regs(BME_CONST.ADDR_RANGE_SW_ERR_ADDR, 1), bits=8)
    ...
    def soft_reset(self):
        ...
        self._set_regs(BME_CONST.SOFT_RESET_ADDR, BME_CONST.SOFT_RESET_CMD)
        time.sleep(BME_CONST.RESET_PERIOD / 1000.0)
    ...
    def set_humidity_oversample(self, value):
        ...
        self._set_bits(BME_CONST.CONF_OS_H_ADDR, BME_CONST.OSH_MSK, BME_CONST.OSH_POS, value)

    def get_humidity_oversample(self):
        """Get humidity oversampling."""
        return (self._get_regs(BME_CONST.CONF_OS_H_ADDR, 1) & BME_CONST.OSH_MSK) >> BME_CONST.OSH_POS
    ...
    def set_pressure_oversample(self, value):
        ...
        self._set_bits(BME_CONST.CONF_T_P_MODE_ADDR, BME_CONST.OSP_MSK, BME_CONST.OSP_POS, value)

    def get_pressure_oversample(self):
        """Get pressure oversampling."""
        return (self._get_regs(BME_CONST.CONF_T_P_MODE_ADDR, 1) & BME_CONST.OSP_MSK) >> BME_CONST.OSP_POS
    ...
    def set_temperature_oversample(self, value):
        ...
        self._set_bits(BME_CONST.CONF_T_P_MODE_ADDR, BME_CONST.OST_MSK, BME_CONST.OST_POS, value)

    def get_temperature_oversample(self):
        """Get temperature oversampling."""
        return (self._get_regs(BME_CONST.CONF_T_P_MODE_ADDR, 1) & BME_CONST.OST_MSK) >> BME_CONST.OST_POS
    ...
    def set_filter(self, value):
        ...
        self._set_bits(BME_CONST.CONF_ODR_FILT_ADDR, BME_CONST.FILTER_MSK, BME_CONST.FILTER_POS, value)

    def get_filter(self):
        """Get filter size."""
        return (self._get_regs(BME_CONST.CONF_ODR_FILT_ADDR, 1) & BME_CONST.FILTER_MSK) >> BME_CONST.FILTER_POS
    ...
    def select_gas_heater_profile(self, value):
        ...
        """
        if value > BME_CONST.NBCONV_MAX or value < BME_CONST.NBCONV_MIN:
            raise ValueError("Profile '{}' should be between {} and {}".format(value, BME_CONST.NBCONV_MIN, BME_CONST.NBCONV_MAX))

        self.gas_settings.nb_conv = value
        self._set_bits(BME_CONST.CONF_ODR_RUN_GAS_NBC_ADDR, BME_CONST.NBCONV_MSK, BME_CONST.NBCONV_POS, value)

    def get_gas_heater_profile(self):
        """Get gas sensor conversion profile: 0 to 9."""
        return self._get_regs(BME_CONST.CONF_ODR_RUN_GAS_NBC_ADDR, 1) & BME_CONST.NBCONV_MSK

    def set_gas_status(self, value):
        """Enable/disable gas sensor."""
        self.gas_settings.run_gas = value
        self._set_bits(BME_CONST.CONF_ODR_RUN_GAS_NBC_ADDR, BME_CONST.RUN_GAS_MSK, BME_CONST.RUN_GAS_POS, value)

    def get_gas_status(self):
        """Get the current gas status."""
        return (self._get_regs(BME_CONST.CONF_ODR_RUN_GAS_NBC_ADDR, 1) & BME_CONST.RUN_GAS_MSK) >> BME_CONST.RUN_GAS_POS
    ...
    def set_gas_heater_temperature(self, value, nb_profile=0):
        ...
        """
        if nb_profile > BME_CONST.NBCONV_MAX or value < BME_CONST.NBCONV_MIN:
            raise ValueError('Profile "{}" should be between {} and {}'.format(nb_profile, BME_CONST.NBCONV_MIN, BME_CONST.NBCONV_MAX))

        self.gas_settings.heatr_temp = value
        temp = int(self._calc_heater_resistance(self.gas_settings.heatr_temp))
        self._set_regs(BME_CONST.RES_HEAT0_ADDR + nb_profile, temp)
    ...
    def set_gas_heater_duration(self, value, nb_profile=0):
        ...
        self._set_regs(BME_CONST.GAS_WAIT0_ADDR + nb_profile, temp)

    def set_power_mode(self, value, blocking=True):
        """Set power mode."""
        if value not in (BME_CONST.SLEEP_MODE, BME_CONST.FORCED_MODE):
            raise ValueError('Power mode should be one of SLEEP_MODE or FORCED_MODE')

        self.power_mode = value

        self._set_bits(BME_CONST.CONF_T_P_MODE_ADDR, BME_CONST.MODE_MSK, BME_CONST.MODE_POS, value)

        while blocking and self.get_power_mode() != self.power_mode:
            time.sleep(BME_CONST.POLL_PERIOD_MS / 1000.0)

    def get_power_mode(self):
        """Get power mode."""
        self.power_mode = self._get_regs(BME_CONST.CONF_T_P_MODE_ADDR, 1)
    ...
    def get_sensor_data(self):
        ...
        """
        self.set_power_mode(BME_CONST.FORCED_MODE)

        for attempt in range(10):
            status = self._get_regs(BME_CONST.FIELD0_ADDR, 1)

            if (status & BME_CONST.NEW_DATA_MSK) == 0:
                time.sleep(BME_CONST.POLL_PERIOD_MS / 1000.0)
                continue

            regs = self._get_regs(BME_CONST.FIELD0_ADDR, BME_CONST.FIELD_LENGTH)

            self.data.status = regs[0] & BME_CONST.NEW_DATA_MSK
            # Contains the nb_profile used to obtain the current measurement
            self.data.gas_index = regs[0] & BME_CONST.GAS_INDEX_MSK
            ...
            gas_range = regs[14] & BME_CONST.GAS_RANGE_MSK

            self.data.status |= regs[14] & BME_CONST.GASM_VALID_MSK
            self.data.status |= regs[14] & BME_CONST.HEAT_STAB_MSK

            self.data.heat_stable = (self.data.status & BME_CONST.HEAT_STAB_MSK) > 0
